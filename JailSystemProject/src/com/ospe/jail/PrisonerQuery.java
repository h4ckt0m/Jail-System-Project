...
public class PrisonerQuery {
	...
	static String filePath = ("src/prisoners.json");

	public static void main(String[] args) {
		int c = 1;
		while (c == 1) {
			realizarConsulta();
			System.out.println("Do you like to add another restriction?(1/0)");
			c = read.nextInt();
			filePath = ("src/Ejemplo_consulta.json");
		}
	...
	public static void realizarConsulta() {
		...
		ArrayList<JSONObject> prisoners = new ArrayList<JSONObject>();
		try {
			...
			for (int j = 0; j < prisoners.size(); j++) {
				...
				JSONObject p = prisoners.get(j);
				consulta.put(p.get("Num_Preso"), p);
				System.out.println("\n");
			}
			try (FileWriter file = new FileWriter("src/Ejemplo_consulta.json")) {

				file.write(consulta.toJSONString());
				file.flush();
