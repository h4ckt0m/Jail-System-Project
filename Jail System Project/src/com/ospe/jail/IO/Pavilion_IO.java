...
public class Pavilion_IO {
	...
	public void leerListado(HashMap<Integer, Pavilion> pavilions) {
		for (Pavilion p : pavilions.values()) {
			System.out.print(
					p.getNum_pabellon() + ", " + p.getNum_celdas() + " celdas, " + p.getNum_presos() + " presos\n");
		}
	}

	public void exportCSV(HashMap<Integer, Pavilion> query, String mainPath) {
		...
		try {
			...
			writer.write("NUM_PABELLON;NUM_CELDAS;NUM_PRESOS;NUM_GUARDIAS;NUM_SALAS_COMUNES;CELDAS CONTENIDAS\n");
	...
	public String stats(HashMap<Integer, Pavilion> pavilions) {
		...
		String s = "\nAverage of cells per pavilion: " + numberFormat.format(mediaCeldas) + " cells"
				+ "\r\nAverage of prisoners per pavilion: " + numberFormat.format(mediaPresos) + " prisoners"
				+ "\r\nAverage of guards per pavilion: " + numberFormat.format(mediaGuardias) + " guards"
				+ "\r\nAverage of community rooms per pavilion: " + numberFormat.format(mediaSalas) + " rooms";
		return s;
	}
}
